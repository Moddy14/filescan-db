...
from utils import write_log, DB_PATH, CONFIG, PROJECT_DIR, logger
...
def fetch_file_data(db_cursor, path_filter=None):
    ...
    query = '''
        SELECT f.file_path, f.size, f.hash, d.path AS dir_path, dr.name AS drive_name
        FROM files f
    ...
    if path_filter:
        ...
        # oder deren *Dateipfad* mit dem path_filter beginnt.
        # Dies ist flexibler als nur LIKE auf file_path.
        normalized_filter = os.path.normpath(path_filter)
        query += " WHERE (d.path LIKE ? OR d.path = ?) OR f.file_path LIKE ?"
        params.extend([normalized_filter + os.sep + '%', normalized_filter, normalized_filter + os.sep + '%'])

    query += " ORDER BY dr.name, d.path, f.file_path" # Bessere Sortierung
...
def export_csv(cursor, filepath):
    ...
    try:
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["Dateipfad", "Größe (Bytes)", "SHA256", "Verzeichnis", "Laufwerk"])
            count = 0
            while True:
...
def export_html(cursor, filepath):
    ...
    try:
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write("  <table>\n")
            f.write("    <thead><tr><th>Dateipfad</th><th>Größe (Bytes)</th><th>SHA256</th><th>Verzeichnis</th><th>Laufwerk</th></tr></thead>\n")
            f.write("    <tbody>\n")
            count = 0
            while True:
                 rows = cursor.fetchmany(1000)
                 if not rows:
                     break
                 for row in rows:
                     f.write("      <tr>" + "".join(f"<td>{html.escape(str(col)) if col is not None else ''}</td>" for col in row) + "</tr>\n")
                 count += len(rows)
            f.write("    </tbody>\n  </table>\n</body>\n</html>")
        # Importiere html erst, wenn es gebraucht wird
        import html
