...
class FSHandler(FileSystemEventHandler):
    ...
    def __init__(self, path_to_watch):
        ...
        # Hintergrund-Drainer: arbeitet gepufferte Events auch dann nach, wenn nach
        # Scan-Lock-Ende KEIN weiteres Watched-Event mehr kommt (Codex-Review P2).
        self._pending_drain_interval = float(CONFIG.get('watchdog_pending_drain_interval', 30))
        self._stop_drainer = threading.Event()
        self._drainer_thread = threading.Thread(
            target=self._pending_drain_loop, name="WatchdogPendingDrainer", daemon=True)
    ...
    def _reinitialize_db_if_needed(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # WICHTIG: close() schliesst die Verbindung UND setzt das Modul-Singleton
                # (models._db_instance=None) zurueck. Ein reines conn.close() wuerde das
                # Singleton auf der toten Instanz belassen -> get_db_instance() liefert sie
                # erneut zurueck und der Watchdog haengt bis zum Service-Neustart (Codex P1).
                try:
                    self.db.close()
                except Exception:
                    pass
    ...
    def _maybe_drain_pending(self):
        """Drained gepufferte Events, wenn welche vorliegen UND das Schreibfenster
        offen ist (kein Scan-Lock). Wird periodisch vom Hintergrund-Drainer aufgerufen,
        damit ein zuletzt waehrend eines Scans gepuffertes Event nicht bis zum naechsten
        Watched-Event oder Service-Neustart haengen bleibt (Codex-Review P2)."""
        try:
            if self._pending and should_fs_write(timeout=0):
                self._drain_pending()
        except Exception as e:
            logger.error(f"[Watchdog Pending] Hintergrund-Drain-Fehler: {e}")

    def _pending_drain_loop(self):
        """Periodischer Hintergrund-Drainer (Daemon-Thread)."""
        while not self._stop_drainer.wait(self._pending_drain_interval):
            self._maybe_drain_pending()

    def stop(self):
        """Stoppt den Hintergrund-Drainer (fuer sauberes Herunterfahren)."""
        try:
